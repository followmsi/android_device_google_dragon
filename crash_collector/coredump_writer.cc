...
ssize_t CoredumpWriter::WriteCoredump(int fd_src,
                                      const std::string& coredump_filename) {
  int fd_dest = TEMP_FAILURE_RETRY(open(coredump_filename.c_str(),
                                        O_WRONLY | O_CREAT | O_EXCL,
                                        S_IRUSR | S_IWUSR));
  if (fd_dest == -1) {
    ALOGE("Failed to open: %s, errno = %d", coredump_filename.c_str(), errno);
    return -1;
  }
  ssize_t result = WriteCoredumpToFD(fd_src, fd_dest);
  close(fd_dest);
  if (result == -1)
...
ssize_t CoredumpWriter::WriteCoredumpToFD(int fd_src, int fd_dest) {
  ...
  if (!GetFileRangeSet(note_buf, &file_range_set)) {
    return -1;
  }
  // Filter out segments backed by mapped files as they are useless when
  // generating minidump.
  std::vector<Phdr> program_headers_filtered;
  FilterSegments(program_headers, file_range_set, &program_headers_filtered);

  // Calculate the output file size.
  const size_t output_file_size = program_headers_filtered.back().p_offset +
      program_headers_filtered.back().p_filesz;
  if (output_file_size > kMaxCoredumpSize) {
    ALOGE("Coredump too large: %d", output_file_size);
    return -1;
  }
...
bool CoredumpWriter::GetFileRangeSet(const std::vector<char>& note_buf,
                                     FileRangeSet* file_range_set) {
  // Locate FILE note.
  ...
  //   ...
  const long* file_count = note.GetDescription().GetData<long>(0);
  if (!file_count) {
  ...
  for (long i = 0; i < *file_count; ++i) {
    const long* start = note.GetDescription().GetData<long>(
        sizeof(long) * (2 + 3 * i));
    const long* end = note.GetDescription().GetData<long>(
        sizeof(long) * (2 + 3 * i + 1));
    if (!start || !end) {
      ALOGE("Invalid FILE Note.");
      return false;
    }
    file_range_set->insert(std::make_pair(*start, *end));
  }
  return true;
}
...
void CoredumpWriter::FilterSegments(
  ...
  for (size_t i = 1; i < program_headers.size(); ++i) {
    ...
    if (out.p_type == PT_LOAD && file_range_set.count(range)) {
      out.p_filesz = 0;
    }
    // Calculate offset.
