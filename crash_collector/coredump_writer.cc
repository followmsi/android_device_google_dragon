...
ssize_t CoredumpWriter::WriteCoredump(int fd_src,
  ...
  ssize_t result = WriteCoredumpToFD(fd_src, fd_dest, proc_files_dir);
  close(fd_dest);
  if (result == -1)
...
ssize_t CoredumpWriter::WriteCoredumpToFD(int fd_src,
                                          int fd_dest,
                                          const std::string& proc_files_dir) {
  ...
  // Write proc files.
  if (!WriteAuxv(note_buf, proc_files_dir + "/auxv") ||
      !WriteMaps(program_headers, file_mappings, proc_files_dir + "/maps")) {
    return -1;
  }
